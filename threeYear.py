...
class ThreeYear(Statistics):
    def __init__(self, dbName1, dbName2, rootDir = None):
        ...
        self.rootDir = os.path.join(self.rootDir, u"3.连续3年+2") 
        if not os.path.exists(self.rootDir):
            os.mkdir(self.rootDir)    
    ...
    def step1(self):
        ...
        for element in self.table1.find({'ji': {'$ne': ""}}):
            ...
            gongLiNianScope = [str(gongLiNian+2), str(gongLiNian+1), str(gongLiNian)]
...
if __name__ == '__main__':
    threeYear = ThreeYear('table1', 'table2')
    threeYear.run()