...
RESTULT_HEADER = "进士数量;进士比例;举人数量;举人比例;武进士数量;武进士比例;翻译进士数量;翻译进士比例;翻译举人数量;翻译举人比例;翻译生员数量;翻译生员比例;其它数量;其他比例;空白数量;空白比例"
...
def precondition():
    ...
    tmpList = []
    for element in res:
        xingMing = element[u"姓名"]
        guanZhiName = element[u"官职名称"]
        key = u"{}-{}".format(xingMing, guanZhiName)
        tmpList.append({key: element})
    logger.info("running precondition, tmpList size = {}".format(len(tmpList)))

    keySet = set([x.keys()[0] for x in tmpList])
    logger.info("running precondition, keySet size = {}".format(len(keySet)))

    resList = [search(tmpList, key) for key in keySet]
    logger.info("running precondition, resList size = {}".format(len(keySet)))

    db['step1'].drop()
    db['step1'].insert(resList)
    return resList


def getFieldValues(fieldName):
    res = db['step1'].distinct(fieldName)
...
def selectWorker(typeName, row):
    keJun = db['step1'].distinct(u"科举功名")

    columnValues = [u"进士", u"举人", u"武进士", u"翻译进士", u"翻译举人", u"翻译生员", ""]
    qiTa = list(set(keJun) - set(columnValues))
    assert len(qiTa) + len(columnValues) == len(keJun)

    logger.info(u"running selectWorker, typeName = {}, row = {}".format(typeName, replaceNullStr(row)))

    jinShi         = list(db['step1'].find({typeName: row, "科举功名": "进士"}))
    juRen          = list(db['step1'].find({typeName: row, "科举功名": "举人"}))
    wuJinShi       = list(db['step1'].find({typeName: row, "科举功名": "武进士"}))
    fanYiJinShi    = list(db['step1'].find({typeName: row, "科举功名": "翻译进士"}))
    fanYiJuRen     = list(db['step1'].find({typeName: row, "科举功名": "翻译举人"}))
    fanYiGongSheng = list(db['step1'].find({typeName: row, "科举功名": "翻译生员"}))
    qiTa           = list(db['step1'].find({typeName: row, "科举功名": {'$in': qiTa}}))
    kongBai        = list(db['step1'].find({typeName: row, "科举功名": ""}))

    return zip(["进士", "举人", "武进士", "翻译进士", "翻译举人", "翻译生员", "其它", "空白"],
               [jinShi, juRen, wuJinShi, fanYiJinShi, fanYiJuRen, fanYiGongSheng, qiTa, kongBai])
