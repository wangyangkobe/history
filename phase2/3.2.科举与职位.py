...
def selectWorker(typeName, row):
    ...
    jinShi         = list(db['step1'].find({typeName: row, "科举功名": {'$in': jinShi}}))
    juRen          = list(db['step1'].find({typeName: row, "科举功名": {'$in': juRen}}))
    belowJuRen     = list(db['step1'].find({typeName: row, "科举功名": {'$in': belowJuRen}}))
...
def generateReport(typeName):
    ...
    baseDir = prepareDir(u"3_2_科举与职位")
